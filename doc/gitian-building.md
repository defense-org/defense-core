...
*Setup instructions for a gitian build of defense using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the defense
...
- [Building defense](#building-defense)
...
Clone the git repositories for defense and gitian and then checkout the defense version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the defense repository
...
To build defense (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the defense repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/defense/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/defense/defense
...
For example:
```bash
URL=https://github.com/konez2k/defense.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit defense=${COMMIT} --url defense=${URL} ../defense/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit defense=${COMMIT} --url defense=${URL} ../defense/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit defense=${COMMIT} --url defense=${URL} ../defense/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/defense-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/defense-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/defense-build.assert
```
...
[defense/gitian.sigs](https://github.com/defense/gitian.sigs/) repository, or if that's not possible to create a pull
request.
