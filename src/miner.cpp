...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("defenseMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("defenseMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("defenseMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("defense-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running defenseMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
